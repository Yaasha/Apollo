...
class AlpacaPaperTrading:
    ...
    def run(self):
        ...
        while True:
            ...
            for position in positions:
              ...
              qty = target_qty
              respSO = []
              tSubmitOrder = threading.Thread(target=self.submitOrder(qty, position.symbol, orderSide, respSO, target_qty))
              tSubmitOrder.start()
              tSubmitOrder.join()
    ...
    def trade(self):
        ...
        if self.turbulence_bool == 0:
            ...
            for index in np.where(action < -min_action)[0]:  # sell_index:
                action[index] *= max(
                    self.stocks[index], self.max_stock
                )  # scale sell action based on available stock  (i.e. do not sell 50 stocks if only 10 are available)

                target_qty = -action[index]
                sell_num_shares = min(self.stocks[index], target_qty)
                qty = abs(sell_num_shares)
                respSO = []
                tSubmitOrder = threading.Thread(
                    target=self.submitOrder(
                        qty, self.stockUniverse[index], "sell", respSO, target_qty
                    )
                )
                tSubmitOrder.start()
                tSubmitOrder.join()
            ...
            for index in np.where(action > min_action)[0]:  # buy_index:
                ...
                action[index] *= min(
                    tmp_cash / self.price[index], self.max_stock
                )  # scale buy action based on available cash  (i.e. do not buy 50 stocks if I can afford only 2)

                target_qty = abs(action[index])
                buy_num_shares = min(tmp_cash / self.price[index], target_qty)
                if buy_num_shares != buy_num_shares:  # if buy_num_change = nan
                    qty = 0  # set to 0 quantity
                else:
                    qty = abs(buy_num_shares)
                qty = abs(buy_num_shares)
                respSO = []
                tSubmitOrder = threading.Thread(
                    target=self.submitOrder(
                        qty, self.stockUniverse[index], "buy", respSO, target_qty
                    )
                )
                tSubmitOrder.start()
                tSubmitOrder.join()
            ...
            for position in positions:
                ...
                tSubmitOrder = threading.Thread(
                    target=self.submitOrder(
                        qty, position.symbol, orderSide, respSO, target_qty
                    )
                )
                tSubmitOrder.start()
                tSubmitOrder.join()
    ...
    def submitOrder(self, qty, stock, side, resp, target_qty):
        ...
        if qty > 0:
            ...
            print(
                f"Quantity is 0 (target {target_qty}), order of | "
                + str(qty)
                + " "
                + stock
                + " "
